...
export default function PricingCard({ plan, price, period = '/mo', features = [], popular = false, ctaText = 'Start Free Trial' }) {
  return (
    <div className={`rounded-2xl p-8 flex flex-col h-full ${popular ? 'bg-rd-teal text-white shadow-2xl scale-105 relative z-10' : 'bg-white text-[#2e384d] shadow-lg border border-gray-100'}`}>
      {popular && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-rd-lime text-rd-dark text-xs font-bold uppercase tracking-wide px-4 py-1 rounded-full">
          Most Popular
        </span>
      )}
      ...
      <ul className="space-y-3 mb-8 flex-1">
        {features.map((feat, i) => (
          <li key={i} className="flex items-start gap-3">
            <svg className={`w-5 h-5 shrink-0 mt-0.5 ${popular ? 'text-rd-lime' : 'text-rd-teal'}`} viewBox="0 0 20 20" fill="currentColor">
